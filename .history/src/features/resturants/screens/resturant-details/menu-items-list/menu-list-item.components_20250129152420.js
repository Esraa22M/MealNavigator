...
export const MenuListItem = ({ item, categoryLength, intialState }) => {
	const [activeCategory, setActiveCategory] = useState({...intialState, isActive:true});
	...
	return (
		<Pressable onPress={handleCategoryNamePress}>
			<View style={styles.itemContainer}>
				<Text style={[item.isActive && styles.activeCategoryName , styles.categoryName]}>{item.categoryName}</Text>
				<View style={[styles.counterContainer, item.isActive && styles.activeContainer]}>
					<Text style={[styles.counter,item.isActive &&styles.activeCounter]}>{item.categoryLength}</Text>
				</View>
			</View>
		</Pressable>
	);
