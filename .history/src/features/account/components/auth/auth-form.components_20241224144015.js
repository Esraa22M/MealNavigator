...
export const AuthForm = ({ isLogin, isValidInputData, inputData }) => {
	...
	const authHandler = async (email, password, mode) => {
		...
		const emailAddress = "esraa22.ing@gmail.com";
		const passwordi = "172003os";
		const modei = "login";
		try {
			setIsLoading(true);
			const token = "";
			if (modei === "login") {
				token = await login(emailAddress.trim(), passwordi.trim());
				console.log(token);
			} else {
				token = await createUser(emailAddress.trim(), passwordi.trim());
				console.log(token);
			}
	...
	console.log(isAuthenticated,"yarab ")
	return (
		<AccountBackground
