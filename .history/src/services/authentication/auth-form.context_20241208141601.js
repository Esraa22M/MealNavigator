...
export const AuthFormContextProvider = ({ children }) => {
	...
	const [isValidLoginData, setIsValidLoginData] = useState({
		email: { isValid: null, icon :"eye"},
		password: { isValid: null, strengthbar: "", icon :"eye"},
	});
	const validateInput = (key, value) => {
		...
		if (key === "email") {
			...
			const iconName = validationObj.isValid?"check-circle-outline"
:"close-circle-outline"

			setIsValidLoginData((prevState) => ({
				...prevState,
				[key]: { ...validationObj ,icon:iconName },
			}));
		} else {
